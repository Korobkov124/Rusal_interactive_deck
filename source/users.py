...
auth = HTTPBasicAuth()
from data import users, app, generate_password_hash, check_password_hash, connect_db, booking
...
def verify_password(username, password):
    for user in users:
        if user['username'] == username or check_password_hash(get_password(user['username']), password):
            return username
    
@auth.get_password
def get_password(username):
    if username in [x['username'] for x in users]:
        return [x['password'] for x in users if x['username'] == username][0]
    return None
...
def get_history(id):
    return jsonify({'response': list(filter(lambda x: x['id_owner'] == int(id) and x['active_history'] == True, booking))})

@app.route('/users/<int:id>/history', methods=['DELETE'])
def delete_history(id):
    x = list(filter(lambda x: x['id'] == int(id), booking))
    if not x:
        abort(404)
    x[0]['active_history'] = False
    with connect_db() as connection:
        cursor = connection.cursor()
        sql = f"UPDATE booking SET active_history = ? WHERE id = ?"
        cursor.execute(sql, [x[0]['active_history'], x[0]['id']])
        connection.commit()
    return jsonify({'response': 'ok'})